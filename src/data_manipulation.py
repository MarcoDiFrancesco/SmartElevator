...
class Features:
    ...
    def subsets(self, features=None):
        ...
        for i in range(1, length):
            # ottiene tutti i subset di lunghezza i (https://www.kite.com/python/answers/how-to-find-all-subsets-with-length-n-of-a-given-set-in-python)
            subset = itertools.combinations(feature_set, i)
            for elem in subset:
                features_combo.append(list(elem))
        return features_combo

    # df è il dataframe, value è  su quale dei dati raccolti  (quale colonna) vogliamo calcolare la feature media, day è il giorno per cui calcoliamo la feature, machine è la macchina per quale calcoliamo la feature
    def mean_calculation(self, df, machine, value, day):
...
class Data_operative:
    def __init__(self, outlier=1, balancing=True, role=None, df=None, features=None):
        ...
        # set role of classifier that use that data
        if role is None:
            self.role = "std"
        else:
            self.role = role
        # set dataframe to stanard if none
        if df is None:
            self.df = self.use_standard_dataframe()
        else:
            self.df = df
        # set features as satandard if none
        if features is None:
            if self.role == "std":
            ...
            for element in self.df.columns.values.tolist():  # update total features
                if (
                    (element != "day")
                    or (element != "machine")
                    or (element != "electricity_1")
                    or (element != "electricity_2")
                    or (element != "magnet")
                    or (element != "bearings")
                ):
            ...
            self.features.set_feature_subject("mean", f)
            self.features.set_feature_subject("var", f)
            self.features.set_feature_subject("skew", f)
            self.features.set_feature_subject("curt", f)
            self.features.set_feature_subject("maxv", f)
            self.features.set_feature_subject("mad", f)
            self.features.set_feature_subject("quant", f)
            self.features.set_feature_subject("iqr", f)
            self.features.set_feature_subject("peak", f)
            self.features.set_feature_subject("trim", f)
            self.features.set_feature_subject("cf", f)
    ...
    def noise_generation(self, df):
        ...
        df_base = df.copy()
        df = df[
            df.label.isin(["bearings-1", "bearings-2", "magnet-partial", "magnet-1"])
        ]
    ...
    def remove_outlier(self, df, outlier=1):
        if outlier == 1:  # first way use 3 sigma
            ...
            IQR = Q3 - Q1
            dff = dff[
                ~((dff < (Q1 - 1.5 * IQR)) | (dff > (Q3 + 1.5 * IQR))).any(axis=1)
            ]
    ...
    def clustering_feature(
        self, df, n_clusters=4, visualize_elbow=False, visualize_clusters=False
    ):
        ...
        # visualize cluster to check elbow method functioning
        # https://www.geeksforgeeks.org/elbow-method-for-optimal-value-of-k-in-kmeans/
        # distortion = It is calculated as the average of the squared distances from the cluster centers of the respective clusters. Typically, the Euclidean distance metric is used.
        # inertia = It is the sum of squared distances of samples to their closest cluster center.
        if visualize_elbow == True:
            distortions = []
            inertias = []
            mapping1 = {}
            mapping2 = {}
            ...
            for k in tqdm(K):
                ...
                distortions.append(
                    sum(
                        np.min(
                            cdist(
                                numeric_df, kmean_model.cluster_centers_, "euclidean"
                            ),
                            axis=1,
                        )
                    )
        ...
        for key, value in features.feature_requested.items():
            if value == True:
                for subject in features.feature_subject[key]:
                    self.df[key + "" + subject] = self.df.apply(
                        lambda x: (
                            features.feature_functions[key](
                                self.df, x["machine"], subject, x["day"]
                            )
                        ),
                        axis=1,
                    )
    ...
    def use_standard_dataframe(self):
        ...
        with open("../28-09_22-11_alldata.json", "r") as f:
            data = json.load(f)
        ...
        df_temp = df_temp.set_index("time").rename(
            columns={"value": "temperature_external"}
        )
        ...
        df.loc[
            (df["time"] >= "2021-10-09 00:00") & (df["time"] <= "2021-10-12 00:00"),
            "label",
        ] = "working-engine-1"
        df.loc[
            (df["time"] >= "2021-10-27 17:30") & (df["time"] <= "2021-10-29 00:00"),
            "label",
        ] = "bearings-1"
        df.loc[
            (df["time"] >= "2021-11-03 16:00") & (df["time"] <= "2021-11-05 11:00"),
            "label",
        ] = "working-engine-2"
        df.loc[
            (df["time"] >= "2021-11-05 13:30") & (df["time"] <= "2021-11-05 23:59"),
            "label",
        ] = "bearings-2"
        # 5th to 10th nothing happened
        df.loc[
            (df["time"] >= "2021-11-10 16:30") & (df["time"] <= "2021-11-16 14:00"),
            "label",
        ] = "magnet-partial"
        df.loc[
            (df["time"] >= "2021-11-16 16:20") & (df["time"] <= "2021-12"), "label"
        ] = "magnet-1"
        ...
        conditions = [
            ...
            (df["label"] == "working-engine-1")
            | (df["label"] == "working-engine-2")
        ...
        for i, row in df.iterrows():
            if (prev_row is not None) and (
                (row.bearings != prev_row.bearings)
                or (row.electricity_1 != prev_row.electricity_1)
                or (row.electricity_2 != prev_row.electricity_2)
                or (row.magnet != prev_row.magnet)
            ):  # row diverse
                marked = 0
            elif (
                row.bearings == 0
            ):  # if all values are same (all defects or no defects at all)
                marked = -1
                start_row.add(row)

            if (
                (row.bearings != 0)
                or (row.electricity_1 != 0)
                or (row.electricity_2 != 0)
                or (row.magnet != 0)
            ):
                final_row.add(row)
            ...
            if counter_measure == 49:
                ...
                df.at[
                    i, "marked"
                ] = marked  # mark if all defects or no defects in 50 measurements
    ...
    def return_data(self, epoch, selected=False, splitted=False):
        ...
        if selected is True:  # if only want selected features
            selected = self.features.selected_features
        else:
            selected = self.features.total_features
        if splitted is False:  # if you want splitting in test and train
    ...
    # QUESTA SINGOLA FUNZIONE DEVO SISTEMARLA DOMANI MATTINA
    # calculate a dataset for rul prediction
    def calculate_rul_label(self, epoch):
        y = self.df[(self.df.day == epoch - 1)]
        y.loc[self.df["magnet"] == 1, "censoring"] = True
        y.loc[self.df["magnet"] == 0.7, "censoring"] = True
        y["censoring"].fillna(False, inplace=True)
        y = y[["censoring", "magnet"]]
        s = y.dtypes
...
class Data_ensemble:
    def __init__(self, models, rul_models, data_operative=None):
        # if no data operative object is passed a standard one is used
        if data_operative is None:
    ...
    def obtain_data(self, epoch, train=True):
        ...
        if train == True:
            ...
            for model in self.rul_models:  # for each rul models
                j = (
                    0 + n_model * 150 + n_rul_model * 100 + n_label * 50
                )  # column number
                i = 0  # row number
                n_label = 0
            ...
            for label in self.data._operative.features.targets:
                df = self.data_operative.df[
                    self.data_operative.df.day == epoch
                ]  # riduco il dataframe all'epoca che stiamo valutando
                i = 0
                j = 0
...
# VARIABLE
# An operative data frame that has a general lookup capacibility
data = Data_operative(role="general")
