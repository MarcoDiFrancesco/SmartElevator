...
class Data_operative:
    ...
    def use_standard_dataframe(self):
        ...
        df.loc[
            (df["time"] >= "2021-11-22 16:30") & (df["time"] <= "2021-11-24 11:00"),
            "label",
        ] = "magnet-2"
        df.loc[
            (df["time"] >= "2021-11-29 17:15") & (df["time"] <= "2021-12-01 12:15"),
            "label",
        ] = "magnet-2-nogrounding"
    ...
    def calculate_rul_label(self, epoch):
        y = self.df[
            (self.df.day <= epoch - 1)
        ]  # take all element before and at that time
        y["censoring"] = False
        y["time"] = 0
        ...
        for i, row in y.iterrows():
            ...
            if (
                row.magnet == 0.7
            ):  # so che ha fallito 0.7 E TEMPORANEO FINCHE NON HO GLI 1
...
class Data_ensemble:
    def __init__(self, models, rul_models, data_operative=None):
        ...
        self.column_number = (
            50 * 3 * (len(self.models)) + 2 * (len(self.rul_models)) * 50
        )
    ...
    def obtain_data(self, epoch, train=True):
        if train == True:
            if epoch == 2:
                ...
                for m in range(0, len(self.models)):
                    for label in ["bearings", "electricity_1", "electricity_2"]:
                        df = self.data_operative.df[
                            self.data_operative.df.day == epoch - 2
                        ]
                        i = 0 + 50 * k * m
                        j = 0
                ...
                for label in self.data_operative.features.targets:
                    df = self.data_operative.df[
                        self.data_operative.df.day == epoch - 1
                    ]  # riduco il dataframe all'epoca che stiamo valutando
                    i = 0
                    j = 0
                    for elem in df[label]:
            ...
            self.data_operative.features.selected_features = list(
                self.X_train.columns.values
            )
