...
class operative_agent_model:
        ...
        if(self.role=='std'):
            if(len(self.dataset.features.total_features)<=4):#if number of features is lower than 4 do all combination
                for features_subset in self.dataset.features.subsets():
                    ...
                    for label in self.dataset.features.targets:
                        ...
                        f1_subset+= self.cross_validate(clf, (["day", "machine"] + features_subset), self.total_score_calculation, label, self.scaler_subset[i], matrix=True)
                        i+=1
                ...
                for elem in feature_obtained:
                    ...
                    for label in self.dataset.features.targets:
                        ...
                        f1_subset+= self.cross_validate(clf, (["day", "machine"] + features_subset), self.total_score_calculation, label, self.scaler_list[i], matrix=True)
                        i+=1
                    f1_subset=f1_subset/3 
            ...
            if(len(self.dataset.features.total_features)<=4):
                for features_subset in self.dataset.features.subsets():
                    ...
                    f1_subset=self.cross_validate(clf, (["day", "machine"] + self.features_subset), self.total_score_calculation, self.dataset.features.targets_nb, self.scaler)
                    if(f1_subset>max_f1):
                        max_f1=f1_subset
                        feature_selected=features_subset
            else:
                ...
                for elem in feature_obtained:
                    ...
                    clf=OneVsOneClassifier(self.model)
                    f1_subset=self.cross_validate(clf, (["day", "machine"] + features_subset), self.total_score_calculation, self.dataset.features.targets_nb, self.scaler, matrix=True)
                    if(f1_subset>max_f1):
                        max_f1=f1_subset
                        feature_selected=features_subset
        ...
        if(self.role=='std'):#for standard case
            if(self.name=="knn"):
                ...
                for param in self.n_neighbors_list:
                    ...
                    i=0
                    for label in self.dataset.features.targets:
                        f1_param+= self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, label, self.scaler_list[i], matrix=True)
                ...
                self.model.set_params(metric=self.metric)
                 ...
                 if(self.name=="knn"):
                     ...
                     for param in self.n_neighbors_list:
                         ...
                         clf.set_params(n_neighbors=param)
                         clf=OneVsOneClassifier(self.model)
                         f1_param=self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, self.dataset.features.targets_nb, self.scaler, matrix=True) 
    ...
    #t is setted with 26 machines  should be changed if increase
     ...
     def calculate_metrics(self):
         ...
         if(self.role=='magnet'):
             self.f1=self.cross_validate(clf, features, self.f1_calculation, target, self.scaler, matrix=True, score=False)
             self.precision=self.cross_validate(clf, features, self.precision_calculation,  target, self.scaler, matrix=True, score=False)
             self.recall=self.cross_validate(clf, features, self.recall_calculation, target, self.scaler, matrix=True, score=False)
             self.accuracy=self.cross_validate(clf, features, self.accuracy_calculation, target, self.scaler, matrix=True, score=False)
             self.corr_coef=self.cross_validate(clf, features, self.corr_coef_calculation, target, self.scaler, matrix=True, score=False)
             self.perc_effort=self.cross_validate(clf, features, self.perc_effort_calculation, target, self.scaler, matrix=True, score=False)
             
             
             if self.probability==True:#metrics that could be calculated only if probabilities are included
                 self.ROC=self.cross_validate(clf, features, self.roc_calculation, target, self.scaler, matrix=False, score=True, list_out=True)
                 self.pr_rec=self.cross_validate(clf, features, self.pr_rec_calculation, target, self.scaler, matrix=False, score=True, list_out=True)
                 self.brier=self.cross_validate(clf, features, self.brier_calculation, target, self.scaler, matrix=False, score=True)

         else:
             i=0
             for label in target:
                 self.f1+=self.cross_validate(clf, features, self.f1_calculation, label, self.scaler_list[i], matrix=True, score=False)
                 i+=1
             self.f1=self.f1/3
             i=0
             for label in target:
                 self.precision+=self.cross_validate(clf, features, self.precision_calculation, label, self.scaler_list[i],  matrix=True, score=False)
                 i+=1
             self.precision=self.precision/3
             i=0
             for label in target:
                 self.recall+=self.cross_validate(clf, features, self.recall_calculation, label, self.scaler_list[i], matrix=True, score=False)
                 i+=1
             self.recall=self.recall/3
             i=0
             for label in target:
                 self.accuracy+=self.cross_validate(clf, features, self.accuracy_calculation, label, self.scaler_list[i], matrix=True, score=False)
                 i+=1
             self.accuracy=self.accuracy/3
             i=0
             for label in target:
                 self.corr_coef+=self.cross_validate(clf, features, self.corr_coef_calculation, label, self.scaler_list[i], matrix=True, score=False)
                 i+=1
             self.corr_coef=self.corr_coef/3
             i=0
             for label in target:
                 self.perc_effort+=self.cross_validate(clf, features, self.perc_effort_calculation, label, self.scaler_list[i], matrix=True, score=False)
                 i+=1
             ...
             if self.probability==True:#metrics that could be calculated only if probabilities are included
                 for label in target:
                     self.ROC.append(self.cross_validate(clf, features, self.roc_calculation, label, self.scaler_list[i], matrix=False, score=True, list_out=True))
                     i+=1
                 i=0
                 for label in target:
                     self.pr_rec.append(self.cross_validate(clf, features, self.pr_rec_calculation, label, self.scaler_list[i], matrix=False, score=True, list_out=True))
                     i+=1
                 i=0
                 for label in target:
                     self.brier+=self.cross_validate(clf, features, self.brier_calculation, label, self.scaler_list[i], matrix=False, score=True)
                     i+=1
     ...
     def performance_calculation(self):
        ...
        if(self.role=='std'):
            ...
            for label in self.dataset.features.targets:
                ...
                self.total_score+= self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, label, self.scaler_list[i], matrix=True)
            ...
            for label in self.dataset.features.targets:
                ...
                self.cl_error+= self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.cl_error_calculation, label, self.scaler_list[i], matrix=True)
            ...
            #preparazione modello
             clf=OneVsOneClassifier(self.model)
             self.total_score=self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, self.dataset.features.targets_nb, self.scaler, matrix=True)
             self.cl_error=self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.cl_error_calculation, self.dataset.features.targets_nb, self.scaler, matrix=True)
             self.confidence=self.calculate_confidence(self.cl_error)
    ...
    #timeseries crossvalidation function
     def cross_validate(self, clf, features, metric, target, scaler=None, matrix=False, score=False, list_out=False):
        value=0
        result=0
        single=True
        if self.role=='magnet':
        ...
        for day in range(1, self.epoch):#E SE ANDASSE CAMBIATO ANCHE ENSEMBLE A EPOCH E NON EPOCH-1
            
            #dataframe extraction for train
            x_train = self.dataset.df[(self.dataset.df.day<(day))]#NON SO SE QUESTO VADA CAMBIATO ANCHE NELL ENSEMBLE
        ...
        if (self.role=='std'):#for standard case
           for scaler1 in scalers:
               for scaler2 in scalers:
                   for scaler3 in scalers: 
                      ...
                      f1_subset=0
                      for label in self.dataset.features.targets:
                          f1_subset+= self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, label, scaler_subset[i], matrix=True)
           ...
           self.scaler_list=selected_normalizer
            ...
            for scaler in scalers:
                     #preparazione modello
                      clf=OneVsOneClassifier(self.model)
                      f1_subset=self.cross_validate(clf, (["day", "machine"] + self.dataset.features.selected_features), self.total_score_calculation, self.dataset.features.targets_nb, self.scaler, matrix=True)
                      if(f1_subset>max_f1):
     ...
     def test_model(self):
          ...
          if(self.role=='std'):#if standard
            ...
            for label in self.dataset.features.targets:  
                ...
                if (self.epoch>2):
                    matrix_list.append(confusion_matrix(self.prev_label[i], self.prev_pred[i]))
                i+=1
            self.f1_pred=self.f1_calculation(self.matrix_list)
              ...
              if(self.epoch>2):
                  matrix_list=multilabel_confusion_matrix(self.prev_label, self.prev_pred)
     ...
     def operative_cycle(self):
        ...
        if(self.role=="std"):#if standard role set classifier with founded hyperparameter
            if(self.name=="logistic"):
                self.model=LogisticRegression(C=self.C, penalty=self.penalty, class_weight=self.weight, solver=self.solver, random_state=0)
            elif(self.name=="svm"):
                ...
                self.model=LinearDiscriminantAnalysis(solver=self.solver, shrinkage=self.shrinkage)
            ...
            if(self.name=="ridge"):
                ...
                self.model=OneVsOneClassifier(KNeighborsClassifier(n_neighbors=self.n_neighbors, weights=self.weights, metric=self.metric))
        ...
        if(self.role=="std"):#if standard role 
            if(self.name=="logistic"):
                self.model=LogisticRegression(tol=self.tol, solver=self.solver, random_state=0)
            elif(self.name=="svm"):
                self.model=svm.SVC(probability=True)       
            elif(self.name=="knn"):
                self.model=KNeighborsClassifier()
