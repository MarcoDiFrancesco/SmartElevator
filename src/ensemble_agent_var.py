...
class ensemble_agent_model:
    def __init__(self, epoch=None, dataset=None, model=None,  model_list=None, rul_model_list=None, opt_function=None):
         ...
         if model_list is None:
             self.model_list=[operative_agent.operative_agent_model('knn', role='std'), operative_agent.operative_agent_model('logistic', role='std'), operative_agent.operative_agent_model('lda', role='std'), operative_agent.operative_agent_model('svm', role='std')]
             self.rul_model_list=[operative_agent.operative_agent_model('knn', role='magnet'), operative_agent.operative_agent_model('svm', role='magnet'), operative_agent.operative_agent_model('ridge', role='magnet')]
         else:
    ...
    def calculate_metrics(self):
        clf = self.model
        self.f1=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.f1_calculation, scaler=None, matrix=True, score=False)
        self.precision=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.precision_calculation, scaler=None, matrix=True, score=False)
        self.recall=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.recall_calculation, scaler=None, matrix=True, score=False)
        self.accuracy=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.accuracy_calculation, scaler=None, matrix=True, score=False)
        self.corr_coef=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.corr_coef_calculation, scaler=None, matrix=True, score=False)
        self.perc_effort=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.perc_effort_calculation, scaler=None, matrix=True, score=False)
        self.ROC=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.roc_calculation, scaler=None, matrix=False, score=True, list_out=True)
        self.pr_rec=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.pr_rec_calculation, scaler=None, matrix=False, score=True, list_out=True)
        self.brier=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, self.brier_calculation, scaler=None, matrix=False, score=True)
    ...
    def performance_calculation(self):
           #model preparation
           clf = self.model
           self.cl_error=self.cross_validate( clf, self.dataset.data_operative.features.selected_features, self.cl_error_calculation, scaler=None, score=False)
    ...
    def scaler_selection(self):
         ...
         for scaler in scalers:
            ...
            clf = self.model
            save_scaler=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, forest_variation.save_calculation, scaler)
    ...
    def cross_validate(self, clf, features, metric, scaler=None, matrix=False, score=False, list_out=False):
        value=0
        result=0
        if list_out:
            value=[]
    ...
    def parameter_selection(self):
             ...
             for parameter in self.estimator_list:
                ...
                save_parameter=self.cross_validate(clf, self.dataset.data_operative.features.selected_features, forest_variation.save_calculation)
    ...
    def feature_selection(self):
            ...
            if(len(list(self.dataset.X_train.columns))<=4):#combinatorial approach
                for features_subset in data_manipulation.data.features.subsets(list(self.dataset.X_train_prev.columns)):
                    ...
                    #model preparation
                    clf = self.model
                    save_subset=self.cross_validate(clf, features_subset, forest_variation.save_calculation)
                ...
                for elem in feature_set:
                    ...
                    #model preparation
                    clf = self.model
                    save_subset=self.cross_validate(clf, feature_subset, forest_variation.save_calculation)
                    if(save_subset>max_save):
                        max_save=save_subset
            ...
            if (len(feature_selected)>(n/2)):
                ...
                self.max_features_list.append(min(int(sqrt(len(feature_selected))*2)), len(X.columns.values))
                self.max_features_list.append(min(int(sqrt(len(feature_selected))*4)), len(X.columns.values))
                self.max_features_list.append(min(int(sqrt(len(feature_selected))*6)), len(X.columns.values))
    ...
    def train_model(self):
         ...
         if(self.name=='RandomForestClassifier'):
             self.model=RandomForestClassifier(n_estimators=self.n_estimators, criterion=self.criterion, min_samples_split=self.min_sample_split, min_samples_leaf=self.min_sample_leaf, bootstrap=self.bootstrap, max_features=self.max_features, random_state=0)#set classifier founded parameter
         else:
             self.model=RandomForestClassifier_variation(n_estimators=self.n_estimators, criterion=self.criterion, min_samples_split=self.min_sample_split, min_samples_leaf=self.min_sample_leaf, bootstrap=self.bootstrap, max_features=self.max_features, random_state=0)#set classifier founded parameter
         self.scaler_selection()
         
         self.performance_calculation()#calculate performance to be expected for next prediction
         self.scaler.fit_transform(self.dataset.X_train)#do a transformation of features befor training
         if(self.name=='RandomForestClassifier'):
             self.model=RandomForestClassifier(n_estimators=self.n_estimators, criterion=self.criterion, min_samples_split=self.min_sample_split, min_samples_leaf=self.min_sample_leaf, bootstrap=self.bootstrap, max_features=self.max_features, random_state=0)#set classifier founded parameter
         else:
             self.model=RandomForestClassifier_variation(n_estimators=self.n_estimators, criterion=self.criterion, min_samples_split=self.min_sample_split, min_samples_leaf=self.min_sample_leaf, bootstrap=self.bootstrap, max_features=self.max_features, random_state=0)#set classifier founded parameter
         self.model.fit(self.dataset.X_train[self.dataset.data_operative.features.selected_features], self.dataset.Y_train.values.ravel())#do train 
    ...
    def test_model(self):
       ...
       if(self.epoch>2):
           self.confusion_matrix=confusion_matrix( self.dataset.Y_train, self.prev_pred)
           self.fi_pred=self.f1_calculation(self.confusion_matrix)
           self.savings+=forest_variation.save_calculation(self.prev_pred, self.dataset.Y_train)
       self.last_pred=self.model.predict(self.dataset.X_test[self.dataset.data_operative.features.selected_features])#do and save prediction
       self.last_pred_proba=self.model.predict_proba(self.dataset.X_test[self.dataset.data_operative.features.selected_features]) #do the same for probabilities


    #operative cycle simulation
    def operative_cycle(self):
        for model in self.model_list:#call an operative cycle for all models inside ensemble
